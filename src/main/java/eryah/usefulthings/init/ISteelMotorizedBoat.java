...
public class ISteelMotorizedBoat extends Item
{
   
    public static Item steel_motorized_boat;

    public ISteelMotorizedBoat()
    {
        this.maxStackSize = 1;
    }
    ...
    public static void init(){
    	steel_motorized_boat = new ISteelMotorizedBoat().setUnlocalizedName("steel_motorized_boat").setCreativeTab(UsefulthingsMod.UTTab);
	}
	
	public static void register()
	{
		GameRegistry.registerItem(steel_motorized_boat, steel_motorized_boat.getUnlocalizedName().substring(5));
	}
	
	public static void registerRenders()
	{
		registerRender(steel_motorized_boat);
	}
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    SteelMotorizedBoat motoboat = new SteelMotorizedBoat(worldIn, (double)((float)blockpos.getX() + 0.5F), (double)((float)blockpos.getY() + 1.0F), (double)((float)blockpos.getZ() + 0.5F));
                    motoboat.rotationYaw = (float)(((MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!worldIn.getCollidingBoundingBoxes(motoboat, motoboat.getEntityBoundingBox().expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return itemStackIn;
                    }
                    ...
                    if (!worldIn.isRemote)
                    {
                        worldIn.spawnEntityInWorld(motoboat);
                    }
